    ...
    def run(self, tmp=None, task_vars=None):
        ...
        def compute_diff(before_str, after_str):
            ...
            def sort_and_key(node):
                ...
                # Sort children using their full keys
                if child_keys:
                ...
                # Calculate Priority Key for THIS node to ensure stable sorting based on ID-like fields.
                # Priority Tags: sequence, name, id, vlan-id, ip.
                # This ensures that an ACL entry with sequence 20 stays at position 20 regardless of other content changes.
                priority_vals = []
                PRIORITY_TAGS = {'sequence', 'name', 'id', 'vlan-id', 'ip'}
                
                for child in node:
                    if child.tag in PRIORITY_TAGS and child.text:
                         val = child.text.strip()
                         # Numeric sort fix for sequence to ensure "2" < "10"
                         if child.tag == 'sequence' and val.isdigit():
                             val = "{:010d}".format(int(val))
                         priority_vals.append(f"{child.tag}:{val}")
                
                priority_key = "|".join(sorted(priority_vals))

                # Generic sort behavior: Append tilde to tag to force base tags to sort after 
                # their prefix-matches (e.g. switchport sorts after switchport-config)
                # 'switchport' < 'switchport-config', but 'switchport~' > 'switchport-config~'
                sort_tag = node.tag + "~"

                my_key = (
                    priority_key, # High priority sort key
                    sort_tag,     # Modified tag for sorting order
